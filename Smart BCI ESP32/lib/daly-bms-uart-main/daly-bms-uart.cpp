...
// Defining the constructor Daly_BMS_UART of the class Daly_BMS_UART
// & is the address-of operator 
// The constructor takes a pass-by-reference (address) to a HardwareSerial object as an input parameter/argument
// "this" is used to refer to the Daly_BMS_UART class, it's to avoid naming clashes (Similar to that in Java).
// this->my_serialIntf is a shorthand for (*this).my_serialIntf, quick and easy way to access the my_serialIntf member of the object (Daly_BMS_UART class) pointed to by the pointer "this".
Daly_BMS_UART::Daly_BMS_UART(HardwareSerial &serial_peripheral)
{
    this->my_serialIntf = &serial_peripheral;
}

// Preprocessor (where header files .h and macros within the source code .cpp is expanded to form the expanded code .i) is the process that runs before the compiler (where the expanded code .i is converted to assembly code .s).
// #ifdef and #endif are preprocessor directives that control conditional compilation 
// (which parts of the source code the compiler will see) based on whether a preprocessor macro is defined.
// Below, it can be read as "If DEBUG_SERIAL is defined via the #define preprocessor macro, keep the code DEBUG_SERIAL.begin(9600); to be included in the compilation process, otherwise discard it before the compiler ever sees it.""
...
bool Daly_BMS_UART::update()
{
    ...
    // If any of them returns false, this update function halts and returns false
...
{
    // do-while iteration: execute the code block once first, then check if the condition is true or not. If true, loop.
