...
void setup()
{
  ...
  tft.getTextBounds(buf, 0, 0, &x1, &y1, &w, &h);
  int16_t txtX = (tft.width()  - w) / 2;
  int16_t txtY = ICON_Y + (ICON_H / 1.65);
  tft.setCursor(txtX, txtY);
  tft.print(buf);
  tft.setFont();                              // restore classic 5×7
  delay(2000);
...
void loop()
{
  ...
  */
  ...
  if (input_soc != previous_soc || input_chg != previous_chg)
  {
    ...
    if (input_soc != previous_soc)
    {
      if (current_status_msg == "BATTERY LOW PLEASE CHARGE" || current_status_msg == "BATTERY FULL!") 
      // Plays every time the SoC changes and the status is still true.
      // (I want "Battery Low. Please Charge" and "Battery Full. Charging Complete" to play every time the SoC changes when the state is still true, for example charging from 95% to 96%, for discharging from 20% to 19%.)
      {
    ...
    if (current_status_msg != previous_status_msg)
    {
