...
void drawStatus(int soc, int chg) 
{
  /* ── Choose the message ─────────────────────────────────────────────── */
  const char *msg = "";
  if (chg && soc < 95)         msg = "BATTERY CHARGING";
  else if (chg && soc >= 95)   msg = "BATTERY FULL!";
  else if (soc <= 20)          msg = "BATTERY LOW PLEASE CHARGE";
...
void tryReadSerialAndUpdateDisplay() 
{
  ...
  drawStatus(input_soc, input_chg); // I too lazy to think of a condition for this

  previous_soc = input_soc;          // update previous values
...
void loop()
{
  tryReadSerialAndUpdateDisplay(); // reads values for the two global variables input_soc and input_chg.
