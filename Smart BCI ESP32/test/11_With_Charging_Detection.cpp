...
constexpr int16_t VOLTAGE_SENSOR = 34;
...
void splashAndHandshake()
{
    ...
    uint8_t goodFrames = 0;            // Count of True returns from bms.update
    uint8_t dot = 0;                   // Count of dots behind "LOADING"

    const char baseMsg[] = "LOADING";

    tft.setFont(&FreeSansBold24pt7b);
    tft.setTextSize(1);
    tft.setTextColor(SOME_LINE_COLOUR, BACKGROUND_COLOUR);

    // Three dots only widen ≤24 px so centring is still fine
    int16_t bx1, by1;
    uint16_t bw, bh;
    tft.getTextBounds(baseMsg, 0, 0, &bx1, &by1, &bw, &bh);
    const int16_t txtX = (tft.width() - bw) / 2;
    const int16_t txtY = ICON_Y + (ICON_H / 2) + (bh / 2);

    uint32_t start = millis();
    while ((millis() - start) < TIMEOUT_MS && goodFrames < 5)
    {
        if (millis() - lastBMSQuery >= BMS_QUERY_MS) // Poll BMS every 250 ms
        {
            bool ok = bms.update();
            lastBMSQuery = millis();
            if (ok)
            ...
            {
                goodFrames = 0; // Need 5 consecutive True returns from bms.update
            }
        }

        static uint32_t lastSpin = 0;
        if (millis() - lastSpin >= 300) // Animate one dot behind "LOADING" every 300 ms
        {
            ...
            msg[8 + dot] = '\0';                                                                               // Terminate
            tft.fillRect(ICON_X + 2, ICON_Y + ICON_H / 2 - bh / 2 - 2, ICON_W - 4, bh + 4, BACKGROUND_COLOUR); // Clear text
            tft.setCursor(txtX, txtY);
            tft.print(msg);
            dot = (dot + 1) % 3; // Three dots at most
            lastSpin = millis();
        }
        delay(2);
    }

    tft.setFont(); // Restore default 5×7 font

    if (goodFrames >= 5) // If 5 consecutive True returns from bms.update, splash and handshake success
    {
        tft.fillScreen(BLACK);
        previous_soc = -1;
        previous_chg = -1;
    ...
    else
    {
        ...
        tft.print("ERROR: NO DATA");
        delay(1000);
        tft.setCursor(30, tft.height() / 1.5);
        tft.print("RETRYING");
        delay(3000);
        tft.fillScreen(BLACK);
        splashAndHandshake(); // Recursion
    }
}
...
void loop()
{
    Serial.println(digitalRead(VOLTAGE_SENSOR));
    delay(100);

    queryBMS();

    input_soc = (int)floorf(bms.get.packSOC); // floor to nearest lower int
    input_chg = digitalRead(VOLTAGE_SENSOR);
